...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Product Recognition Training", formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    ...
    parser.add_argument("--dataset", type=str, help="dataset (format name_attr e.g. shape)", required=True)
    ...
    parser.add_argument(
        "--optimizer", type=str, help="optimizer (adam, sgd or lars)", choices=["adam", "sgd", "lars"], default="sgd"
    )
    ...
    parser.add_argument(
        "--method", type=str, help="loss function", choices=["infonce", "infonce-strong"], default="infonce"
    )
    ...
    parser.add_argument("--epsilon", type=float, help="infonce epsilon", default=0.0)
    parser.add_argument("--lr_epsilon", type=float, help="epsilon lr", default=1e-4)

    parser.add_argument("--lambd", type=float, help="lagrangian weight for debiasing", default=0.0)
    parser.add_argument("--lambd_alpha_ratio", type=float, help="compute lambd as alpha*ratio (0=disabled)", default=0)
    parser.add_argument("--kld", type=float, help="weight of std term", default=1.0)

    parser.add_argument("--alpha", type=float, help="infonce weight", default=1.0)
...
def build_data(cfg):
    if cfg.dataset not in ["danube", "shape"]:
...
def build_optimizer(model, criterion, cfg):
    ...
    if "auto" in cfg.method:
        parameters.append({"params": criterion.epsilon, "lr": cfg.lr_epsilon, "weight_decay": 0})

    if cfg.optimizer == "sgd":
        optimizer = torch.optim.SGD(parameters, lr=cfg.lr, momentum=cfg.momentum, weight_decay=cfg.weight_decay)
    ...
    if cfg.lr_decay == "cosine":
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=cfg.epochs, verbose=True)
    ...
    if cfg.beta == 0:
        ...
        if cfg.mlp_optimizer == "sgd":
            optimizer_fc = torch.optim.SGD(fc_params, lr=cfg.mlp_lr, momentum=0.9, weight_decay=0)
        elif cfg.mlp_optimizer == "adam":
            optimizer_fc = torch.optim.Adam(fc_params, lr=cfg.mlp_lr, weight_decay=0)

        if cfg.mlp_lr_decay == "cosine":
            scheduler_fc = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_fc, T_max=cfg.epochs, verbose=True)
        else:
            scheduler_fc = None
...
def measure_similarity(feat, labels, bias_labels):
    ...
    negative_conflicting_sim_mean = (negative_conflicting_sim.sum(dim=1)) / (
        torch.count_nonzero(neg_conflicting, dim=1) + 1
    )

    # print(f"pos-aligned: {pos_aligned.sum() - bsz}, pos-conflicting: {pos_conflicting.sum() - bsz}")
    return (
        (aligned_sim[torch.nonzero(pos_aligned, as_tuple=True)], aligned_sim_mean.mean()),
...
def test(test_loader, model, criterion, cfg):
    ...
    for images, labels, bias_labels in test_loader:
        ...
        with torch.no_grad():
            ...
            (
                (al_sim, al_sim_mean),
                (con_sim, con_sim_mean),
                (neg_al_sim, neg_al_sim_mean),
                (neg_confl_sim, neg_confl_sim_mean),
...
if __name__ == "__main__":
    ...
    run_name = (
        f"{method}_{cfg.form}_{cfg.dataset}_{cfg.backbone}_"
    ...
    if cfg.lambd != 0:
        ...
        def infonce_fairkl(projected, feats, logits, labels, bias_labels):
            ...
            return cfg.alpha * target_loss(projected, labels) + cfg.lambd * losses.fairkl(
                feats, labels, bias_labels, 1.0, kld=cfg.kld
            )
